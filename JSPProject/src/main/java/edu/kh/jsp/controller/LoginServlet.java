...
//		out.println("</html>");
		...
		// webapp 폴더 내부에 존재하는 WEB-INF폴더->views폴더->loginResult.jsp파일 선택
		
		RequestDispatcher dispatcher=req.getRequestDispatcher("/WEB-INF/views/loginResult.jsp");
		
		// ** forward **
		// 보내다, 전달하다, 전송하다
		// HttpsServletRequest, HttpServletResponse를 매개변수로 전달
		// 왜? JSP가 요청데이터(req)를 이용해서 화면을 만들고
		// 클라이언트에게 응답(resp)하기 위해서 매개변수로 전달
		
		// forward는 페이지 이동이 아닌
		// Servlet의 역할 중 응답화면 출력 역할을 분업한
		// JSP에게 req,resp을 전달만 하는 것
		// -> JSP가 응답을 대신 할 뿐이다
		...
		req.setAttribute("r", result); // Object로 업캐스팅 되어있는 상태
